...
function get_addon_list()
{
    ...
    foreach ($results as $name) {
        ...
        //// 这里不采用get_addon_info是因为会有缓存(忽略没有*.ini文件目录)
...
function get_addon_autoload_config($truncate = false)
{
    ...
    $route = [];
    // 读取插件目录及钩子列表
    $base = get_class_methods("\\think\\Addons"); //vendor/karsonzhang/fastadmin-addons/src/Addons.php插件基类;
    ...
    foreach ($addons as $name => $addon) {
        ...
        foreach ($hooks as $hook) {
            $hook = Loader::parseName($hook, 0, false); //解析转换名称;
...
/**
 * 获取插件类的配置值
 * @param string $name 插件名(如third第三方登录)
 * @return array
 *
 * @author ^2_3^
 */
...
function addon_url($url, $vars = [], $suffix = true, $domain = false)
{
    ...
    $val = "@addons/{$url}";
    $config = get_addon_config($addon); //获取配置信息;
