...
class FormBuilder
{
    ...
    public function input($type, $name, $value = null, $options = [])
    {
        ...
        if (!in_array($type, $this->skipValueTypes)) {
            ...
            $options['class'] = isset($options['class']) ?
                $options['class'] . (stripos($options['class'], 'form-control') !== false ? '' :
    ...
    public function selectpage($name, $value, $url, $field = null, $primaryKey = null, $options = [])
    {
        $options = array_merge($options,
            [
                'data-source' => $url, 'data-field' => $field ? $field : 'name',
    ...
    public function switcher($name, $value, $options = [])
    {
        ...
        $disabled = (isset($options['disabled']) && $options['disabled']) || in_array('disabled', $options) ?
            "disabled" : '';
        ...
        $html = <<<EOD
{$btn}
<a href="javascript:;" data-toggle="switcher" class="btn-switcher {$disabled}" data-input-id="c-{$domname}" 
    data-yes="{$yes}" data-no="{$no}" {$attr}><i class="fa fa-toggle-on text-{$color} {$selected} fa-2x"></i></a>
EOD;
        return $html;
    }
...
EOD;
    ...
    public function cxselect($url, $names = [], $values = [], $options = [])
    {
        ...
        foreach ($names as $index => $value) {
            ...
            $cxselect[] = <<<EOD
<select class="{$class} form-control" name="{$value}" data-value="{$selectValue}" 
    data-url="{$url}?level={$level}&name={$value}" {$attributes}></select>
EOD;
        }
...
EOD;
    ...
    public function checkboxs($name, $list, $checked, $options = [])
    {
        ...
        foreach ($list as $k => $v) {
            ...
            $html[] = sprintf(
                ...
                Form::checkbox($name, $k, in_array($k, $checked), $options)
            );
        }
    ...
    protected function uploader(
        $name = null, $value, $inputAttr = [], $uploadAttr = [], $chooseAttr = [], $previewAttr = []
    )
    {
        $domname = str_replace(['[', ']', '.'], '', $name);
        $options = [
        ...
        $previewArea = $preview ? '<ul class="row list-inline plupload-preview" id="p-' . $domname . '" ' .
            $previewAttrHtml . '></ul>' : '';
...
EOD;
    ...
    public function attributes($attributes)
    {
        ...
        // 假设我们的key 和 value 是相同的,
        // 拿HTML的“required”属性来说,假设是['required']数组,
        // 会以 required="required" 拼接起来,而不是用数字key去拼接
...
class Arr
{
    ...
    /**
     * Get an item from an array using "dot" notation(符号).
     *
     * @param  \ArrayAccess|array $array
     * @param  string $key
     * @param  mixed $default
