...
class Random
{
    ...
    /**
     * 能用的随机数生成
     * @param string $type 类型 alpha/alnum(字母数字)/numeric/nozero(无零数字)/unique/md5/encrypt/sha1；
     * @param int $len 长度；
     * @return string
     * @author ^2_3^
     */
    public static function build($type = 'alnum', $len = 8)
    {
        switch ($type)
        {
            ...
            case 'nozero':
                ...
                // str_shuffle() 打乱一个字符串，使用任何一种可能的排序方案。
                return substr( str_shuffle( str_repeat($pool, ceil( $len / strlen($pool) ) ) ), 0, $len );

            case 'unique':
            case 'md5':
                // mt_rand — 生成更好的随机数；
                // uniqid — 生成一个唯一ID，第二参数为true则后面加上小数值；
    ...
    public static function lottery($ps, $num = 1, $unique = true)
    {
        ...
        foreach ($ps as $key => $value)
        {
            ...
            // strchr — 别名 strstr()，返回查找字符串的首次出现到结尾的部分；
