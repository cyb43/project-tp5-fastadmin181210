 ...
 *      第三个参数为and时表示，用户需要同时具有规则1和规则2的权限。当第三个参数为or时，表示用户只需要具备其中一个条件即可，默认为or。
 * 3，一个用户可以属于多个用户组(fa_auth_group_access表，定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(fa_auth_group，
 *      定义了用户组权限)。
 * 4，支持规则表达式。
 *      在fa_auth_rule 表中定义一条规则，condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100，
...
class Auth
{
    ...
    public function check($name, $uid, $relation = 'or', $mode = 'url')
    {
        ...
        // $name 需要验证的规则列表,支持逗号分隔的权限规则或索引数组；
        if (is_string($name))
        {
    ...
    public function getRuleList($uid)
    {
        ...
        //// 用户会话权限(登录验证) [认证方式，1为实时认证；2为登录认证]
        ...
        foreach ($this->rules as $rule)
        {
            ...
            {
                // 根据condition进行验证(会员验证)
                $user = $this->getUserInfo($uid); //获取用户信息,一维数组；
                // condition字段可以定义规则表达式。 如定义{score}>5 and {score}<100，表示用户的分数在5-100之间时这条规则才会通过。
                // $user['score'] > 5 and $user['score'] < 100
